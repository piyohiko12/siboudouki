  ...
  const PICKER = [
    ...
    },
    {
      ...
      options: [
        ...
        {
          label: function (isJob) {
            return isJob ? '入社後に挑戦したいことが、はっきりしている' : '入学後に挑戦したいことが、はっきりしている';
          },
          note: 'これからの意欲を軸に書く書き方が向いています',
          vote: { gap: 2 }
        },
        {
    ...
    return [
      ...
      // ─────────────────── その志望先と自分が合っていると思う理由
      {
        ...
        title: isJob
          ...
          : 'その学校と自分が合っていると思う理由',
        lead: (isJob
          ? 'ここまで書いた会社の魅力に、自分のどこが重なるのかをさがします。'
          : 'ここまで書いた学校の魅力に、自分のどこが重なるのかをさがします。')
          + '答えるのは単語だけで大丈夫。'
        ...
        groups: [
          ...
          },
          {
            id: 'gap',
            name: 'これから挑戦したいこと',
            desc: 'この型は、「入ったあとに挑戦したいこと」から書き始めます。'
          }
        ],
        fields: [
          {
          ...
          },
          {
            ...
            label: isJob ? '入社後に挑戦したいこと' : '入学後に挑戦したいこと',
            required: true,
            only: ['gap'],
            placeholder: isJob ? '新しい機械の操作' : '地域の課題を調べる研究',
            examples: isJob
              ? ['新しい機械の操作', '品質を高める工夫', '資格をとって仕事の幅を広げること']
              : ['地域の課題を調べる研究', '海外への留学', '資格をとって専門を深めること'],
            hint: 'この構成は、ここが出発点になります。'
              + 'いまできないことではなく、「入ってからやってみたいこと」を書きます。'
              + 'ものごとの名前でも、「〜すること」の形でも構いません。',
            avoid: '「がんばりたい」だけでは、何に挑戦したいのかが伝わりません',
            preview: function (d) {
              const t = txt(d.gapNow);
              if (!t) return '';
              if (isWish(t)) return '私はこれから、' + plainWord(t) + 'と考えています。';
              return frame(t,
                '私がこれから挑戦したいのは、{X}です。',
      ...
      // ───────────────────────────── 学校／会社を知る
      {
        ...
        no: 3,
        title: 'そこに決めた理由',
        lead: 'ここがこの文章の心臓部です。いま書いたきっかけを思い浮かべながら、'
          + '「心が動いた瞬間」を思い出してください。',
      ...
      // ───────────────────────────── 調べる
      {
        id: 'research',
        no: 2,
        title: isJob ? '会社を選んだきっかけ' : '学校を選んだきっかけ',
        lead: isJob
          ? 'その会社をどこで知って、どこにひかれたのかをはっきりさせます。ここが薄いと、どの会社にも出せる文章になってしまいます。'
          : 'その学校をどこで知って、どこにひかれたのかをはっきりさせます。ここが薄いと、どの学校にも出せる文章になってしまいます。',
        note: isJob
        ...
        groups: [
          ...
          {
            id: 'found',
            name: isJob ? 'この会社にひかれたところ' : 'この学校にひかれたところ',
            desc: isJob
